...
CREATE TABLE IF NOT EXISTS `sfm`.`Student` (
  ...
  `StudyStatus` VARCHAR(30) NULL COMMENT 'Đang Học, Bảo Lưu',
  `SubMajor` VARCHAR(45) NULL,
  `Term` VARCHAR(10) NULL COMMENT 'K7, K8, K9',
  `NarrowSpecialization` VARCHAR(20) NULL COMMENT 'Chuyên Nghành hẹp :\n-Normal\n-IS\n-JS\n- ES',
...
CREATE TABLE IF NOT EXISTS `sfm`.`SubtractAmount` (
  `SubtractAmountID` INT NOT NULL AUTO_INCREMENT,
  `SubtractAmountName` VARCHAR(45) NULL,
  `SubtractAmount` MEDIUMTEXT NULL,
  PRIMARY KEY (`SubtractAmountID`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `sfm`.`PaidAmount`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `sfm`.`PaidAmount` ;

CREATE TABLE IF NOT EXISTS `sfm`.`PaidAmount` (
  `PaidAmountID` INT NOT NULL AUTO_INCREMENT,
  `PaidAmount` MEDIUMTEXT NULL,
  PRIMARY KEY (`PaidAmountID`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `sfm`.`Session_Tuition` (
  ...
  `SubtractAmountID` INT NOT NULL COMMENT 'Cac khoan khau tru',
  `RealPayMoney` MEDIUMTEXT NOT NULL,
  `PaidMoneyID` INT NOT NULL,
  ...
  INDEX `fk_paid_amount_idx` (`PaidMoneyID` ASC),
  ...
  CONSTRAINT `fk_subtract_amount`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_paid_amount`
    FOREIGN KEY (`PaidMoneyID`)
    REFERENCES `sfm`.`PaidAmount` (`PaidAmountID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
